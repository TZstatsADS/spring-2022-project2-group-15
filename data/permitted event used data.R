...
#choose data from 2019 to 2022
data_final <- data_use %>%
  mutate(Year = substr(`Start Date/Time`,7,10)) %>%
  mutate(Month = substr(`Start Date/Time`,1,2 )) %>%
  mutate(Date = substr(`Start Date/Time`,4,5 )) %>%
  dplyr::select(`Event ID`,`Event Name`, Year, Month, Date, `Event Agency`, `Event Type`, `Event Borough`, `Event Location`) %>%
  arrange(Year,Month,Date) %>%
  filter(Year >= 2019) %>%
  distinct()

write.table(data_final,"data_use_final.csv",row.names=FALSE,col.names=TRUE,sep=",")